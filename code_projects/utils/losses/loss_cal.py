...
def multiclass_loss(pred, gth: torch.Tensor, cls, ignore: int, ce_weight, lovasz_weight, dice_weight):
    ...
    class_weights = torch.tensor([0.25, 1.5, 1.5, 1.2, 1.2, 1.2, 1.2,
                                  ...
                                  1.0, 0.8, 0.8, 0.8], dtype=torch.float32).to(pred.device)
    ...
    dice_score = 0.0
    if dice_weight > 0.0:
        dice_loss_fn = DiceLoss(weight=class_weights, reduction='mean',ignore_index=ignore)
...
def final_loss(model_name, pred, gth: torch.Tensor, classes: int,
             ce_bce_weight, lovasz_weight, dice_weight, device, ignore: int = 255):
    ...
    if classes == 2:
        ...
        if model_name == "EfficientNetb0_UNet3Plus":
            ...
            w = [0.3, 0.2, 0.2, 0.2, 0.1]
            for i in range(len(pred)):
                loss = multiclass_loss(pred[i], gth, classes, ignore,
                                       ce_bce_weight, lovasz_weight, dice_weight)
                loss_list.append(loss)
            for i in range(len(loss_list)):
                score += loss_list[i] * w[i]
        else:
            score = multiclass_loss(pred, gth, classes, ignore,
                                    ce_bce_weight, lovasz_weight, dice_weight)
        return score
    else:
